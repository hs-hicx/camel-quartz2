 ...
 * @version 
 */
public class Quartz2Endpoint extends DefaultEndpoint implements ShutdownableService {
    private static final transient Logger LOG = LoggerFactory.getLogger(Quartz2Endpoint.class);
    ...
    public Quartz2Endpoint(final String endpointUri, final Quartz2Component component) {
        super(endpointUri, component);
        ((JobDetailImpl)getJobDetail()).setName("quartz-" + getId());
    }
    ...
    public void addTrigger(final Trigger trigger, final JobDetail detail) throws SchedulerException {
        ...
        if (trigger.getKey().getName() == null) {
        	(( AbstractTrigger<?>) trigger).setName(detail.getKey().getName());
        }
        // lets default the trigger group to the job group
        if (trigger.getKey().getGroup() == null) {
        	(( AbstractTrigger<?>) trigger).setGroup(detail.getKey().getGroup());
        }
        // default start time to now if not specified
        if (trigger.getStartTime() == null) {
        	(( AbstractTrigger<?>) trigger).setStartTime(new Date());
        }
        detail.getJobDataMap().put(Quartz2Constants.QUARTZ_ENDPOINT_URI, getEndpointUri());
        // must use management name as it should be unique in the same JVM
        detail.getJobDataMap().put(Quartz2Constants.QUARTZ_CAMEL_CONTEXT_NAME, getCamelContext().getManagementName());
        if (detail.getJobClass() == null) {
        	((JobDetailImpl)detail).setJobClass(isStateful() ? StatefulCamelJob.class : CamelJob.class);
        }
        if (detail.getKey().getName() == null) {
        	((JobDetailImpl)detail).setName(getJobName());
        }
    ...
    public void deleteTrigger(final Trigger trigger) throws SchedulerException {
        getComponent().deleteJob(trigger.getKey().getName(), trigger.getKey().getGroup());
    }
    ...
    public Exchange createExchange(final JobExecutionContext jobExecutionContext) {
        ...
        exchange.setIn(new Quartz2Message(exchange, jobExecutionContext));
        return exchange;
    }
    ...
    public Quartz2Consumer createConsumer(Processor processor) throws Exception {
        return new Quartz2Consumer(this, processor);
    }

    @Override
    protected String createEndpointUri() {
        return "quartz2://" + getTrigger().getKey().getGroup() + "/" + getTrigger().getKey().getName();
    }
    ...
    @Override
    public Quartz2Component getComponent() {
        return (Quartz2Component) super.getComponent();
    }
    ...
    public synchronized void consumerStarted(final Quartz2Consumer consumer) throws SchedulerException {
    ...
    public synchronized void consumerStopped(final Quartz2Consumer consumer) throws SchedulerException {
        ObjectHelper.notNull(trigger, "trigger");
        if (started) {
